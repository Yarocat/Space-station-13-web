...
export function canEquipInSlot(itemType: ItemType, slot: SlotName): boolean {
  ...
  if (slot === 'head') {
    return ['Helmet', 'Tophat', 'CaptainCap', 'Hardhat', 'WeldingMask'].includes(itemType);
  }
  if (slot === 'mask') {
    return ['GasMask', 'SterileMask'].includes(itemType);
  }
  if (slot === 'glasses') {
  ...
  if (slot === 'suit') {
    return ['Armor', 'SpaceSuit', 'FireSuit', 'Labcoat'].includes(itemType);
  }
  if (slot === 'uniform') {
  ...
  const handleTileClick = (x: number, y: number, isRightClick?: boolean, screenPos?: { x: number; y: number }) => {
    ...
    if (isAdjacent) {
      ...
      if (closetObj) {
        ...
        SoundSystem.door(closetObj.isOpen);
