...
export const SPECIES_CATALOG: SpeciesTrait[] = [
  ...
  {
    type: 'Vulpakanin',
    name: 'Vulpakanin (Paradise SS13)',
    origin: 'Altam Vulpis System',
    lore: 'Canine-vulpine humanoids from Paradise Station renowned for agility, fluffy bushy tails, keen scent tracking, and digitigrade anatomy.',
    perks: ['Superior low-light night vision', 'Natural claw defense (+1 dmg)', 'Padded paws: quiet movement', 'Paradise SS13 DMI sprites'],
  ...
  {
    type: 'Tajaran',
    name: 'Tajaran (Paradise SS13 / Ahdomai)',
    origin: 'Ahdomai Cold World',
    lore: 'Graceful feline race from Paradise Station with warm thick coats, sharp retractile claws, sensitive whiskers, and nimble athletic reflexes.',
    perks: ['Dense fur grants cold resistance (+50K tolerance)', 'Razor claws (+2 unarmed damage)', 'Feline leap balance', 'Paradise SS13 DMI sprites'],
...
export const CharacterSetupModal: React.FC<CharacterSetupModalProps> = ({
  ...
  const [dmiVersion, setDmiVersion] = useState<number>(0);

  useEffect(() => {
    return addDmiRedrawListener(() => setDmiVersion((v) => v + 1));
  }, []);
  ...
  useEffect(() => {
    ...
    // 1. Species base body and tail layering
    const baseDmi = getSpeciesDmiInfo(species, skinTone, gender);

    // Tail: behind body when facing SOUTH, EAST, or WEST
    if (baseDmi.tailDmi && previewDir !== 'NORTH') {
      drawDmiSprite(ctx, baseDmi.tailDmi, baseDmi.tailState || '', 0, 0, { dir: previewDir });
    }
    ...
    // Tail: in front of body when facing NORTH
    if (baseDmi.tailDmi && previewDir === 'NORTH') {
      drawDmiSprite(ctx, baseDmi.tailDmi, baseDmi.tailState || '', 0, 0, { dir: previewDir });
    }

    // Species ears accessory (Tajaran & Vulpakanin)
    if (baseDmi.earsDmi) {
      drawDmiSprite(ctx, baseDmi.earsDmi, baseDmi.earsState || '', 0, 0, { dir: previewDir });
    }

    // 2. Additional ears & tail fallback if DMI not yet loaded
    if (!drawn && (species === 'Vulpakanin' || species === 'Tajaran')) {
