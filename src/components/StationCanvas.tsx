    ...
    const render = () => {
      ...
      for (let y = startY; y <= endY; y++) {
        for (let x = startX; x <= endX; x++) {
          ...
          // Classic Griefly / TGStation DMI turf rendering
          const turfDmi = getTurfDmiInfo(tile.turf, map, x, y);
          let drawn = drawDmiSprite(ctx, turfDmi.dmi, turfDmi.state, tx, ty);
          if (!drawn) {
            drawn = drawTurfRsiSprite(ctx, tile.turf, tx, ty);
          }
        ...
        for (let x = startX; x <= endX; x++) {
          ...
          for (const obj of tile.objects) {
            if (obj.isItem) {
              ...
              // Classic Griefly / TGStation DMI Structure & Door rendering
              ...
              let drawn = drawDmiSprite(ctx, structDmi.dmi, structDmi.state, tx, ty, {
                frame: structDmi.frame,
              });
              if (!drawn) {
                drawn = drawStructureRsiSprite(ctx, obj, tx, ty);
              }
      ...
      if (player.isGhost) {
        ...
        // Pure SS13 / Griefly Composite DMI Paperdoll Mob
        drawDmiMob(ctx, player, px, py);
      }
